...
export function computeGeometry({
  ...
  const contentWidth =
    renderedSlideCount > 0
      ? 2 * peek + renderedSlideCount * slideWidth + (renderedSlideCount - 1) * spacing
      : 0;
...
export function pageFromOffset(offset: number, geometry: Geometry): ResolvedOffset {
  const unit = unitFromOffset(offset, geometry);
  if (!geometry.cloned) {
    return { page: clamp(unit, 0, geometry.pageCount - 1), unit, onClone: false };
  }
  if (unit === 0) {
...
export const pageForSlide = (slide: number, geometry: Geometry, slideCount: number): number =>
  clamp(
    Math.floor(clamp(Math.floor(slide) || 0, 0, Math.max(0, slideCount - 1)) / geometry.visibleSlides),
    0,
 ...
 */
export function sourceIndexFor(renderedIndex: number, geometry: Geometry, slideCount: number): number {
...
export function withClones<T>(items: readonly T[], geometry: Geometry): readonly T[] {
  if (!geometry.cloned) {
    return items;
  }
