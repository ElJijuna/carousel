...
const MockArrow = ({ direction, onPress, disabled, accessibilityLabel }: CarouselArrowSlotProps) => (
  <Pressable
...
describe('rendering', () => {
  ...
  it('resolves a responsive visibleSlides map against its own width', async () => {
    await render(
      <Carousel testID="c" visibleSlides={{ base: 3, 700: 2, 400: 1 }} components={{ Dot: MockDot }}>
        {slides(6)}
      </Carousel>,
    );
...
describe('paging', () => {
  ...
  it('reports a page change once, not once per scroll frame', async () => {
    ...
    for (const x of [280, 300, 310, 300]) {
      await fireEvent.scroll(track, { nativeEvent: { contentOffset: { x, y: 0 } } });
    }
...
describe('autoPlay', () => {
  ...
  it('resumes when the finger lifts', async () => {
    ...
    const track = screen.getByTestId('c-track');
    await fireEvent(track, 'scrollBeginDrag', { nativeEvent: { contentOffset: { x: 0, y: 0 } } });
    await fireEvent(track, 'scrollEndDrag', { nativeEvent: { contentOffset: { x: 0, y: 0 } } });
...
describe('data mode', () => {
  const photos = Array.from({ length: 6 }, (_, i) => ({ id: `p${i}`, title: `photo ${i}` }));
...
describe('useCarousel', () => {
  const Counter = () => {
    ...
    return (
      <Pressable testID="counter" onPress={() => next()} accessibilityState={{ disabled: !canGoNext }}>
  ...
  it('renders every slot wherever it is configured', async () => {
    await render(
      <Carousel
        ...
        components={{ Arrow: MockArrow, Dot: MockDot, PlayPauseControl: MockPlayPause }}
      >
        {slides(3)}
      </Carousel>,
    );
