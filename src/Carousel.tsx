...
const webSnapSlide = (isPageStart: boolean): ViewStyle | null =>
  IS_WEB ? ({ scrollSnapAlign: isPageStart ? 'start' : 'none' } as ViewStyle) : null;
...
const isCloneAt = (renderedIndex: number, geometry: Geometry, slideCount: number) =>
  geometry.cloned &&
  (renderedIndex < geometry.leadingClones || renderedIndex >= geometry.leadingClones + slideCount);
...
function CarouselImpl<TItem>(props: CarouselProps<TItem>, ref: Ref<CarouselHandle>): ReactElement {
  ...
  const previous = useCallback(
    (options?: CarouselNavigateOptions) => navigate(-1, options, wraps, 'previous'),
    [navigate, wraps],
  );
  const goTo = useCallback(
    (
      ...
      applyTarget(goToTarget(target, geometry), options?.animated ?? true, source);
    },
  ...
  const imperativeNext = useCallback(
    (options?: CarouselNavigateOptions) => navigate(1, options, wraps, 'imperative'),
    [navigate, wraps],
  );
  ...
  const imperativeGoTo = useCallback(
    (target: number, options?: CarouselNavigateOptions) => goTo(target, options, 'imperative'),
    [goTo],
  );
  const imperativeGoToSlide = useCallback(
    (slide: number, options?: CarouselNavigateOptions) => goToSlide(slide, options, 'imperative'),
    [goToSlide],
  );
  ...
  useImperativeHandle(
    ...
    [imperativeNext, imperativePrevious, imperativeGoTo, imperativeGoToSlide, play, pause, pageRef],
  );
