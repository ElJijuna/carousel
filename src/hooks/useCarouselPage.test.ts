...
const setup = async (initialProps: UseCarouselPageOptions) =>
  renderHook((props: UseCarouselPageOptions) => useCarouselPage(props), { initialProps });
...
describe('uncontrolled', () => {
  ...
  it('reports once per actual change, not once per scroll frame', async () => {
    ...
    await act(async () => {
      ...
      expect(result.current.commitPage(1)).toBe(true);
      expect(result.current.commitPage(1)).toBe(false);
      expect(result.current.commitPage(1)).toBe(false);
    });
  ...
  it('treats a NaN commit as page 0', async () => {
    ...
    await act(async () => {
      result.current.commitPage(Number.NaN);
    });
...
describe('controlled', () => {
  ...
  it('follows the prop when the parent does feed it back', async () => {
    const onPageChanged = jest.fn();
    const { result, rerender } = await setup({ pageCount: 5, page: 1, onPageChanged });
