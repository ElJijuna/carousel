...
describe('onScroll', () => {
  ...
  it('does nothing before the first layout pass', async () => {
    const { result, commitPage } = await setup({ geometry: geometryOf({ containerWidth: 0 }) });
...
describe('infinite clone handling', () => {
  const infiniteGeometry = geometryOf({ slideCount: 6, visibleSlides: 2, infinite: true });
  ...
  it('leaves a landing on a real page alone', async () => {
    const { result, scroller } = await setup({ geometry: infiniteGeometry, page: 0 });
...
describe('re-anchoring', () => {
  ...
  it('does not undo clone travel when the page prop catches up', async () => {
    const infinite = geometryOf({ slideCount: 6, visibleSlides: 2, infinite: true });
    const { result, scroller, rerender, initialProps } = await setup({
      geometry: infinite,
      page: 2,
    });
...
describe('scroller shapes', () => {
  it('drives a FlatList through scrollToOffset', async () => {
    ...
    const view = await renderHook(
      (props: UseCarouselScrollOptions) => useCarouselScroll(props),
      {
        initialProps: {
          geometry: geometryOf(),
          page: 0,
    ...
    expect(scrollToOffset).toHaveBeenCalledWith({ offset: 300, animated: false });
  });
