...
test('a user scroll snaps to a page and the chrome follows', async ({ page }) => {
...
test('peek leaves the neighbouring slides showing at the edges', async ({ page }) => {
...
test('the track snaps on page boundaries, not on every slide', async ({ page }) => {
  await openStory(page, 'peek-and-spacing');

  const alignments = await page.evaluate(() =>
    Array.from(document.querySelectorAll('[data-testid^="slide-"]')).map(
      (slide) => getComputedStyle(slide.parentElement as HTMLElement).scrollSnapAlign,
    ),
  );

  // Two slides per page, so every second slide is a snap point. Snapping on
  // each one instead would let a drag rest mid-page.
  expect(alignments.filter((value) => value === 'start').length).toBeGreaterThan(0);
  expect(alignments[0]).toBe('start');
  expect(alignments[1]).toBe('none');
});

test('a resize re-finds the page instead of keeping a stale pixel offset', async ({ page }) => {
  await openStory(page, 'basic');
