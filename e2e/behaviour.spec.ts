...
test.describe('infinite', () => {
  ...
  test('renders a clone page at each end, hidden from assistive tech', async ({ page }) => {
    await openStory(page, 'infinite');
  ...
  test('wraps forward past the end and lands back on page 0', async ({ page }) => {
    await openStory(page, 'infinite');
  ...
  test('wraps backward past the start and lands on the last page', async ({ page }) => {
    await openStory(page, 'infinite');
...
test.describe('autoPlay', () => {
  test('advances on its own and the control stops it', async ({ page }) => {
    ...
    await expect.poll(() => scrollLeft(page), { timeout: 8000 }).toBeGreaterThan(0);
  ...
  test('resumes when the control is pressed again', async ({ page }) => {
    ...
    await expect.poll(() => scrollLeft(page), { timeout: 8000 }).not.toBe(paused);
  });
});
...
test.describe('controlled', () => {
  ...
  test('a move inside the carousel is reported back to the parent', async ({ page }) => {
    await openStory(page, 'controlled');
...
test.describe('headless contract', () => {
  ...
  test('custom chrome built on useCarousel drives the same state', async ({ page }) => {
    await openStory(page, 'custom-chrome-via-hook');

    const fillWidth = () =>
      page.evaluate(
        () => (document.querySelector('[data-testid="progress-fill"]') as HTMLElement).offsetWidth,
      );
...
test.describe('responsive', () => {
  test('regroups the pages when the carousel is given less room', async ({ page }) => {
